...
import by.bsuir.carapp.ui.components.CarList
...
fun HomeScreen(isFav: Boolean, auth: AuthenticationService, navController: NavHostController) {
    val (selectedCar, setSelectedCar) = remember { mutableStateOf<Car?>(null) }
    ...
    Scaffold(
        ...
        topBar = {
            HomeTopBar(
                isFav,
                if (!isFav) "Все машины" else "Избранное",
        ...
        content = { padding ->
            ...
            if (showBottomSheet) {
                ModalBottomSheet(
                    ...
                    BottomSheetContent(car = selectedCar)
                }
            }
        }
...
fun BottomSheetContent(car: Car?) {
    car?.let {
        CarDetailed(car = it, folderPath = "${it.name}/detailed_photos")
    }
}