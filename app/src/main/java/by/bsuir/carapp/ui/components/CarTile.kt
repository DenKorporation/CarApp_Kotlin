...
import by.bsuir.carapp.data.*
import by.bsuir.carapp.services.DatabaseService
import by.bsuir.carapp.viewmodel.PreviewImageViewModel


@Composable
fun CarTile(onlyFavourites: Boolean, car: Car, userData: UserData?, onCarSelected: (Car) -> Unit) {
    car.isFav = userData?.favCars?.contains(car.uid) ?: false

    if (onlyFavourites && !car.isFav) return
